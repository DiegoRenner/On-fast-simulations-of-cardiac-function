...
Nevertheless the accuracy has been shown to be sufficient to simulate haemodynamics in large vascular systems.
While the increase in simulation speed gained by choosing 1D models over 3D models is significant, an efficient implementation of the solver is still important.
...
Usually vessels that are at the edge of the network and that have an inlet that is not ocnnected to another vessel are considered to be fed by the heart.
Here the inlet is set through precomputed data.
This data either represents the pressure $P_L(t)$ or the volumetric flow $Q_L(t)$ being put out by the heart
\begin{align}
...
The time step size is defined as described in section \ref{sec:cfl_condition},
...
\subsubsection{Outlets} \label{sssec:oulets}
...
\section{CFL Condition} \label{sec:cfl_condition}
...
\end{equation}
Three of the models are taken from the \emph{vascularmodel.com} \cite{vascularmodel} website.
...
The input data provided by \emph{vascularmode.com} was used.
...
The 56 vessels are split into 61 arterial segments which are in turn split into 77 segments for simulation purposes 
...
\begin{figure} [!ht]
	\centering
	\includegraphics[width=0.8\columnwidth]{figures/0007_annotated.png}
...
\begin{figure} [!ht]
	\centering
	\includegraphics[width=0.8\columnwidth]{figures/0029_annotated.png}
...
\begin{figure} [!ht]
	\centering
	\includegraphics[width=0.8\columnwidth]{figures/0053_annotated.png}
...
If the compilation time doesn't increase much even for larger models this would mean that for big enough systems the compilation overhead would pay off.
...
\begin{figure} [!ht]
	...
	\caption{The compile time doesn't grow significantly with increasing numbers of vessel segments due to builtin Python loops being avoided which would be costly to compile.}
	\label{fig:timing}
\end{figure}
The compile time remains almost constant around 3s even as when the number of vessel segments increases over two magnitudes of order.
...
In general the compile time is only a fraction of the total compute time amd becomes insignificant with growing model sizes.
