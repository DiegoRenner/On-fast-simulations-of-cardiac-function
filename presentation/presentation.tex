	...
	\begin{block}{What to look out for:}
		\begin{itemize}
			...
			\item<3-> avoiding for loops (loops in general)
				\begin{itemize}
					\item<4-> Python for-loop: slow compilation
					\item<5-> JAX for-loop: homogeneous mesh 
					\item<6-> $\rightarrow$ avoid for-loops through vectorization
					\item<7-> $\rightarrow$ requires padding and masking
				\end{itemize}
		\end{itemize}
	\end{block}
\end{frame}
