...
\begin{frame}<1>[label=mp]
	\only<1>{\frametitle{3D Model Pipeline}}
	\only<2->{\frametitle{1D Model Pipeline}}
	\vspace{-10pt}
	\begin{figure}[htbp]
		\only<-99>{\begin{minipage}[t][0.06\paperheight][t]{\linewidth}
			...
			\end{minipage}
			\hfill
			\only<1>{\begin{minipage}{0.18\linewidth}
				\caption*{\tiny}
		\end{minipage}}
			\only<2->{\begin{minipage}{0.18\linewidth}
	...
	\end{minipage}}
		...
		\begin{minipage}[t][0.06\paperheight][t]{\linewidth}
			...
			\hfill
			\begin{minipage}{0.18\linewidth}
				\onslide<1->{\caption*{\tiny Biomarkers}}
			\end{minipage}
			\hfill
			\begin{minipage}{0.1\linewidth}
				\caption*{\tiny}
...
\includegraphics[width=\linewidth]{images/adan56_tibiofibular_trunk_L_P.eps}
		\end{minipage}}
			\hfill
		\only<-99>{\begin{minipage}{0.1\linewidth}
			\caption*{\tiny}
	\end{minipage}}
...
\begin{frame}<1>[label=ic]
	\frametitle{Initial Conditions (IC)}
		\onslide<2->{\begin{minipage}{0.37\textwidth}
	...
	\end{minipage}}
		\hfill
		\onslide<4->{\begin{minipage}{0.57\textwidth}
...
\begin{frame}
	...
	\begin{block}{Main Points of Interest}
		\begin{itemize}
			...
			\item generalize code for junctions involving $N$ vessels
			\item stable parameter inference
			\item sensitivity analysis (global and local)
		\end{itemize}
	\end{block}
	\vspace{5mm}
\end{frame}
